...
let getTrainingSampleLines () =
    File.ReadAllLines("D:\\tmp\\trainingsample.csv")

let getValidationSampleLines () =
    File.ReadAllLines("D:\\tmp\\validationsample.csv")

let trainingSampleLines = getTrainingSampleLines ()
let validationSampleLines = getValidationSampleLines ()
...
let trainingSampleMatrixWithHeader = trainingSampleLines |> Array.map (fun (line:string) -> line.Split(','))  
let validationSampleMatrixWithHeader = validationSampleLines |> Array.map (fun (line:string) -> line.Split(','))  
...
let trainingSampleStringMatrix = trainingSampleMatrixWithHeader.[1..trainingSampleMatrixWithHeader.Length - 1];
let validationSampleStringMatrix = validationSampleMatrixWithHeader.[1..validationSampleMatrixWithHeader.Length - 1];
...
let trainingSampleMatrix =
    trainingSampleStringMatrix |> Array.map(
        fun (line) -> line |> Array.map(fun (column) -> Convert.ToInt32(column)));

let validationSampleMatrix =
    validationSampleStringMatrix |> Array.map(
        fun (line) -> line |> Array.map(fun (column) -> Convert.ToInt32(column)));
...
let trainingSampleRecords =
    trainingSampleMatrix |> Array.map(
        fun (line) -> { Label  = line.[0]; Pixels = line.[1..line.Length-1] });

let validationSampleRecords =
    validationSampleMatrix |> Array.map(
        fun (line) -> { Label  = line.[0]; Pixels = line.[1..line.Length-1] });
...
let distance (p1: int[]) (p2: int[]) =
    sqrt ((Array.map2 (fun x1 x2 -> (x1-x2)*(x1-x2)) p1 p2) |> Array.sum |> float)
...
let classify (unknown:int[]) =
    trainingSampleRecords 
    |> Array.minBy (fun record -> distance record.Pixels unknown)
...
let results = validationSampleRecords |> Array.map (fun record -> (classify record.Pixels).Label = record.Label)
let successCount = (Array.filter (fun value -> value = true) results).Length;
